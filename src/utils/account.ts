...
if (address.length !== 42 && address.length !== 64) return { error: 'Invalid address' };
    const id = shardus.getNodeId();
    const isInRotationBonds = shardus.isNodeInRotationBounds(id);
if (isInRotationBonds) return { error: 'node close to rotation edges' };
if (!req.query.type) {
        let shardusAddress = address.toLowerCase();
if (address.length === 42) shardusAddress = toShardusAddress(address, AccountType.Account);
...
if (AccountType[accountType] == null) return { error: 'Invalid account type' };
        const secondaryAddressStr = (req.query.secondaryAddress || '').toString();
if (secondaryAddressStr && secondaryAddressStr.length !== 66) return { error: 'Invalid secondary address' };
