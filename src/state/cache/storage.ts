...
if (addressStoragePreState === undefined) diffStorageMap = new Map();
else diffStorageMap = addressStoragePreState;
...
if (oldStorageMap) oldStorage = oldStorageMap.get(keyHex);
            }
else {
...
if (!storageMap) storageMap = new Map();
...
if (this._lruCache) storageMap = this._lruCache!.get(addressHex);
else storageMap = this._orderedMapCache!.getElementByKey(addressHex);
        this._stats.reads += 1;
if (storageMap) {
            this._stats.hits += 1;
