...
if (address.equals(this._env.address)) return this._env.contract.balance;
...
if (original) return this._stateManager.originalStorageCache.get(this._env.address, key);
else return this._stateManager.getContractStorage(this._env.address, key);
    }
    /**
...
if (this.common.consensusAlgorithm() === ConsensusAlgorithm.Clique) coinbase = this._env.block.header.cliqueSigner();
else coinbase = this._env.block.header.coinbase;
...
return BigInt(0);
        // EIP-2681 check
if (this._env.contract.nonce >= MAX_UINT64) return BigInt(0);
...
if (results.execResult.exceptionError) return BigInt(0);
