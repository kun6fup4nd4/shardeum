...
export const handlers: Map<number, OpHandler> = new Map([
    ...
    [
        ...
        function (runState): void {
            const [a, b] = runState.stack.popN(2);
            let r;
if (b === BigInt(0)) r = BigInt(0);
else r = mod(a / b, TWO_POW256);
            runState.stack.push(r);
        },
    ],
    // 0x05: SDIV
    [
        ...
        function (runState): void {
            ...
            let r;
if (b === BigInt(0)) r = BigInt(0);
else r = toTwos(fromTwos(a) / fromTwos(b));
            runState.stack.push(r);
        },
    ],
    // 0x06: MOD
...
if ((val >> signBit) & BigInt(1)) val = val | BigInt.asUintN(256, ~mask);
else val = val & mask;
            }
...
if (a > 256) {
if (isSigned) r = MAX_INTEGER_BIGINT;
else r = BigInt(0);
                runState.stack.push(r);
return;
            }
...
if (length !== BigInt(0)) data = runState.memory.read(Number(offset), Number(length));
...
if (loaded.length < 32) r = r << (BigInt(8) * BigInt(32 - loaded.length));
            runState.stack.push(r);
        },
    ],
    // 0x36: CALLDATASIZE
...
if (runState.env.versionedHashes.length > Number(index))
    ...
    [
        ...
        async function (runState): Promise<void> {
            ...
            let value;
if (val === BigInt(0)) value = Uint8Array.from([]);
else value = bigIntToBytes(val);
...
if (memLength !== BigInt(0)) mem = runState.memory.read(Number(memOffset), Number(memLength));
...
if (common.isActivatedEIP(3860) &&
            ...
            let data = new Uint8Array(0);
if (length !== BigInt(0)) data = runState.memory.read(Number(offset), Number(length), true);
...
if (length !== BigInt(0)) data = runState.memory.read(Number(offset), Number(length), true);
    ...
    [
        ...
        async function (runState: RunState): Promise<void> {
            ...
            let data = new Uint8Array(0);
if (inLength !== BigInt(0)) data = runState.memory.read(Number(inOffset), Number(inLength), true);
            const gasLimit = runState.messageGasLimit!;
            runState.messageGasLimit = undefined;
...
if (memLength > BigInt(128)) memLength = BigInt(128);
            let mem = runState.memory.read(Number(memOffset), Number(memLength));
if (mem.length < 128) mem = setLengthRight(mem, 128);
...
return;
    ...
    [
        ...
        async function (runState): Promise<void> {
            ...
            runState.messageGasLimit = undefined;
            let data = new Uint8Array(0);
if (argsLength !== BigInt(0)) data = runState.memory.read(Number(argsOffset), Number(argsLength));
...
if (length !== BigInt(0)) returnData = runState.memory.read(Number(offset), Number(length));
