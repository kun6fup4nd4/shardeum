...
if (exponent === BigInt(0)) return gas;
            let byteLength = exponent.toString(2).length / 8;
if (byteLength > Math.trunc(byteLength)) byteLength = Math.trunc(byteLength) + 1;
...
if (account === undefined || account.isEmpty()) deadAccount = true;
...
if (account === undefined || account.isEmpty()) deductGas = true;
                }
            }
