...
if (opts.blockchain === undefined) blockchain = new DefaultBlockchain();
else blockchain = opts.blockchain;
...
if (!this.common.isActivatedEIP(3540)) result = { ...result, ...INVALID_BYTECODE_RESULT(message.gasLimit) };
...
if (typeof eof1CodeAnalysisResults?.code === 'undefined') result = {
                        ...result,
...
if (!EOF.validOpcodes(result.returnValue.subarray(codeStart, codeStart + eof1CodeAnalysisResults.code))) result = {
                            ...result,
                            ...INVALID_EOF_RESULT(message.gasLimit),
                        };
else
...
if (!contract) contract = new Account();
...
if (!callerAccount) callerAccount = new Account();
...
if (message.depth === 0) {
if (!callerAccount) callerAccount = await this.stateManager.getAccount(message.caller);
if (!callerAccount) callerAccount = new Account();
            callerAccount.nonce++;
...
if (this.common.isActivatedEIP(3540))
    ...
    protected async _generateAddress(message: Message): Promise<Address> {
        let addr;
if (message.salt) addr = generateAddress2(message.caller.bytes, message.salt, message.code as Uint8Array);
else {
