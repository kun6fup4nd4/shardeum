...
function multComplexity(x: bigint): bigint {
    let fac1;
    let fac2;
if (x <= BigInt(64)) return x ** BigInt(2);
...
function getAdjustedExponentLength(data: Uint8Array): bigint {
    ...
    let max32expLen = 0;
if (expLen < BigInt(32)) max32expLen = 32 - Number(expLen);
...
if (expLenMinus32OrZero < BigInt(0)) expLenMinus32OrZero = BigInt(0);
...
if (power === BigInt(0)) return BigInt(1) % modulo;
    let res = BigInt(1);
...
if (adjustedELen < BigInt(1)) adjustedELen = BigInt(1);
...
if (!opts.common.isActivatedEIP(2565)) gasUsed = (adjustedELen * multComplexity(maxLen)) / Gquaddivisor;
else {
        gasUsed = (adjustedELen * multComplexityEIP2565(maxLen)) / Gquaddivisor;
if (gasUsed < BigInt(200)) gasUsed = BigInt(200);
    }
