...
else if (account.accountType === AccountType.Debug) hash = crypto.hashObj(account);
else if (account.accountType === AccountType.ContractStorage) hash = crypto.hashObj({ key: account.key, value: account.value });
else if (account.accountType === AccountType.ContractCode) hash = crypto.hashObj({ key: account.codeHash, value: account.codeByte });
else if (account.accountType === AccountType.Receipt) hash = crypto.hashObj({ key: account.txId, value: account.receipt });
...
if (fromStr.length === 42) fromStr = fromStr.slice(2); //trim 0x
