...
if (account.accountType === AccountType.NodeRewardReceipt) return account.ethAddress;
        const shardusAddress = toShardusAddress(addressSource, account.accountType);
return shardusAddress;
    }
...
else if (account.accountType === AccountType.SecureAccount) return (account as unknown as SecureAccount).id.toLowerCase();
}
...
if (addressStr.length === 66) return addressStr.slice(2).toLowerCase();
else {
...
if (secondaryAddressStr.length === 66) secondaryAddressStr = secondaryAddressStr.slice(2);
...
if (addressStr.length === 64) return addressStr.toLowerCase();
else {
            throw new Error('must pass in a 64 character hex addressStr AccountType.Receipt');
        }
if (accountType === AccountType.Receipt ||
