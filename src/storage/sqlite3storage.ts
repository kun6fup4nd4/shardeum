...
if (opts && opts.createOrReplace) queryString = table.insertOrReplaceString;
            const inputs = [];
            ...
            for (const column of table.columns) {
                ...
                // eslint-disable-next-line security/detect-object-injection
if (table.isColumnJSON[column]) value = Utils.safeStringify(value);
                // if (logFlags.console) console.log(`column: ${column}  ${value}`)
                inputs.push(value);
            }
            queryString += this.options2string(opts);
            // if (logFlags.console) console.log(queryString + '  VALUES: ' + stringify(inputs)) return this.run(queryString, inputs);
        }
        finally {
...
return results;
    ...
    _update(table, values, where, opts): Promise<unknown> {
        try {
            ...
            queryString += this.options2string(opts);
            // if (logFlags.console) console.log(queryString + '  VALUES: ' + stringify(valueArray)) return this.run(queryString, valueArray);
        }
        finally {
            //this.profiler.profileSectionEnd('db')
        }
...
if (optionsObj === null || optionsObj === undefined) return '';
        let optionsString = '';
if (optionsObj.order) {
