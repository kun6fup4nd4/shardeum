...
if (nodeAccount.nominator == null) return;
    // check if nodeAccount.rewardStartTime is already set to eventData.time
if (nodeAccount.rewardStartTime >= tx.nodeActivatedTime) return;
...
if (!tx.nominee || tx.nominee === '' || tx.nominee.length !== 64) return { success: false, reason: 'invalid nominee field in setRewardTimes Tx' };
if (!tx.nodeActivatedTime) return { success: false, reason: 'nodeActivatedTime field is not found in setRewardTimes Tx' };
if (tx.nodeActivatedTime < 0 || tx.nodeActivatedTime > shardeumGetTime()) return { success: false, reason: 'nodeActivatedTime is not correct in setRewardTimes Tx' };
...
if (!shardus.serviceQueue.containsTxData(tx.txData)) return { success: false, reason: 'node not in serviceQueue' };
    // check txData matches tx
if (tx.txData.startTime !== tx.nodeActivatedTime) return { success: false, reason: 'txData.startTime does not match nodeActivatedTime' };
if (tx.txData.publicKey !== tx.nominee) return { success: false, reason: 'txData.publicKey does not match tx.nominee' };
...
if (nodeAccount.rewardStartTime >= tx.nodeActivatedTime) return { result: 'fail', reason: 'rewardStartTime is already set' };
if (nodeAccount.timestamp >= tx.timestamp) return { result: 'fail', reason: 'timestamp is already set' };
return { result: 'pass', reason: 'valid' };
}
...
if (isNodeAccount2(acct)) nodeAccount = acct;
else
...
if (wrappedStates[networkAccount]?.data && isNetworkAccount(wrappedStates[networkAccount].data)) network = wrappedStates[networkAccount].data as NetworkAccount;
