...
if (fromAccount === undefined) fromAccount = new Account();
...
if (this.common.isActivatedEIP(1559) === true) {
    ...
    // EIP-4844 tx
    let versionedHashes;
if (tx instanceof BlobEIP4844Transaction) versionedHashes = (tx as BlobEIP4844Transaction).versionedHashes;
...
else txCost = tx.gasLimit * gasPrice;
...
else actualTxCost = results.totalGasSpent * gasPrice;
...
if (this.DEBUG)
    ...
    let miner;
if (this.common.consensusType() === ConsensusType.ProofOfAuthority) miner = block.header.cliqueSigner();
else miner = block.header.coinbase;
    let minerAccount = await state.getAccount(miner);
if (minerAccount === undefined) minerAccount = new Account();
